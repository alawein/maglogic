...
export GITHUB_REPOSITORY="alawein/maglogic"   # optional; defaults to alawein/maglogic
node scripts/github-sync-report.mjs
```

Artifact: `reports/sync-report.maglogic.json` (gitignored). In **GitHub Actions**, run workflow **"Ops — GitHub sync report"** (workflow_dispatch); it uploads the JSON as a workflow artifact.
